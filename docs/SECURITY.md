...
OneKey data enters through a server-only provider adapter. The browser never receives the BBO bearer key or MLS Grid token. BBO keys should be dedicated to Homix Marketing and scoped to read-only listing/events plus the exact listing-scoped Agent and marketing-recipient routes. The key cannot search the full Agent roster. Automatic Agent sync validates an Active roster status and email, links identity by stable BBO member key, rejects email/source conflicts, allows only HTTPS headshots and never falls back to an unrelated local Agent. Provider errors are sanitized, searches/limits are bounded, and remote media is copied only from the configured provider origin, size-checked, decoded by Sharp and stored under Homix control.
