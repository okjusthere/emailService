...
Campaign DTO sender fields include `batchSize`, `minBatchIntervalSeconds`, `timezone`, `sendWindowStart`, `sendWindowEnd`, and `allowedWeekdays`. `dailyLimit` is a positive integer or `null` for no daily cap; consumers must display null as “No daily cap” rather than zero. Sender-level global pacing and the send window apply equally to Portal and native Campaigns, including queued Campaigns. Homix uses one email every 60 seconds, every day 08:00–18:00 in `America/New_York`, without a daily cap or warm-up quota.
